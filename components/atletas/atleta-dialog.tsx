...
export function AtletaDialog({ equipas }: AtletaDialogProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await createAtleta(validData);
      setFormData({ nome: "", numero: "", posicao: "", equipaId: "" });
      setIsOpen(false);
    } catch (err) {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-md">
        <h2 className="text-xl font-bold text-slate-900 mb-4">Novo Atleta</h2>
