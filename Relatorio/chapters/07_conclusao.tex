...
Por outro lado, ficaram a descoberto algumas limitações que devem ser reconhecidas. O sistema de notificações ficou limitado a alertas \textit{in-app}, sem integração com email/SMS, por não termos investido tempo a configurar provedores externos. Felizmente, lacunas graves que existiam inicialmente --- como a ausência de \textbf{hashing de palavras-passe} e a falta de uma \textbf{suite de testes automatizados} --- foram ultrapassadas com sucesso na iteração final, onde implementámos \texttt{BCrypt.Net-Next} para garantir a confidencialidade (RNF-01) e \texttt{xUnit} para robustecer a API.
...
\begin{enumerate}
    ...
    \item \textbf{Expansão da suite de testes}: expandir os testes unitários atuais para outros serviços de negócio (especialmente regras como \texttt{isApto()} ou cálculo de IMC) e testes de integração end-to-end, executados em CI no GitHub.
